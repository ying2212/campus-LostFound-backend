...
export async function updatePost(req, res) {
    const { id } = req.params;
    const { title, Description, ImageUrl, Category, Foundat, Timefound, Status, HowToClaim } = req.body
    const data = {};
    ...
    try {
        ...
        if (existingPost.authorId !== req.user.id) {
            // If not author, only allow Status update (claiming)
            if (Object.keys(req.body).length === 1 && req.body.Status !== undefined) {
                // Allow
            } else if (Object.keys(req.body).length > 1 && req.body.Status !== undefined) {
                // If trying to update other things along with Status, strip them or error?
                // For safety, let's error if they try to touch anything else
                return res.status(403).json({ error: "You can only claim this post, not edit other fields" });
            } else {
